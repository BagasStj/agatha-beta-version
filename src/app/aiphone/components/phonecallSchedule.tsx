...
import { PhoneIcon, MicIcon, PhoneOffIcon, Settings, Phone, Brain, BookUser, Table, Table2, ClockIcon, Hourglass } from 'lucide-react';
...
export default function PhoneCallSchedule() {
    ...
    try {
      ...
      const callResponse = await fetch('https://api.vapi.ai/call/phone', {
        ...
        body: JSON.stringify({
          ...
          customer: {
            ...
            // number: '+6287729723438' // Add +62 prefix here
          },
          phoneNumber: {
      ...
      toast({
        ...
        description: scheduledTime
          ? `Scheduled call started at ${dayjs().format('DD MMMM YYYY HH:mm:ss')}`
  ...
  }, [toast, vapiClient, contact, user, scheduledTime]);

  const handleSubmit2 = useCallback(async (data: PhoneCallFormData) => {
    if (scheduledTime) {
      const now = dayjs();
      const scheduledTimeUTC = scheduledTime.utc();
      const timeUntilCall = scheduledTimeUTC.diff(now);
      ...
      if (timeUntilCall > 0) {
        ...
        toast({
          title: "Invalid Time",
          description: "Please select a future time for the call.",
          duration: 3000,
          variant: "destructive",
        });
      }
    ...
    <div className="space-y-6 p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold flex items-center">
          <Hourglass className="mr-2" /> Phone Call Settings with Schedule
        </h2>
        ...
        <Tabs defaultValue="llm-model" className="w-full" onValueChange={setActiveTab}>
          <div className="flex justify-between items-center mb-4">
            ...
            <Button
              ...
              disabled={isCallActive || isCallScheduled}
            >
              {isCallActive ? (
                <>
                ...
                <>
                  <ClockIcon className="mr-2 h-4 w-4" /> Call Scheduled
                </>
              ) : (
                <>
                  <PhoneIcon className="mr-2 h-4 w-4" /> {scheduledTime ? 'Schedule Call' : 'Call Now'}
                </>
              )}
            </Button>
          </div>
              ...
              <div>
                ...
                <TimePicker
                  ...
                  onChange={(time) => {
                    if (time) {
                      const now = dayjs();
                      const scheduledDateTime = now.hour(time.hour()).minute(time.minute()).second(0);
                  ...
                  value={scheduledTime}
                  disabled={isCallActive}
                  className="w-full" // Menambahkan kelas untuk lebar penuh
                  style={{ width: '50%' }}
                />
              </div>
              <div>
