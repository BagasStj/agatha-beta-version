...
interface HeaderComponentProps {
  currentUser: any;
  onLogout: () => void;
  currentView: 'users' | 'chat' | 'email' | 'product' | 'novel' | 'flow' | 'aiphone' | 'chat-flowise' | 'chat-new' | 'ai-voice';
  onViewChange: (view: 'users' | 'chat' | 'email' | 'product' | 'novel' | 'flow' | 'aiphone' | 'chat-flowise' | 'chat-new' | 'ai-voice') => void;
}
...
export function HeaderComponent({ currentUser, onLogout, currentView, onViewChange }: HeaderComponentProps) {
  ...
  const handleViewChange = (view: 'users' | 'chat' | 'email' | 'product' | 'novel' | 'flow' | 'aiphone' | 'chat-new' | 'ai-voice') => {
    onViewChange(view);
    setIsOpen(false);
  };
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent side="left" className="w-[300px] sm:max-w-none">
          ...
          <nav className="space-y-4" aria-label="Main Navigation">
            {['users', 'chat', 'email', 'product', 'novel', 'flow', 'ai-voice'].map((view) => (
              <Link
                key={view}
                href="#"
                onClick={() => handleViewChange(view as 'users' | 'chat' | 'email' | 'product' | 'novel' | 'flow' | 'aiphone' | 'chat-new' | 'ai-voice')}
        ...
        <BreadcrumbList>
          ...
          {currentView === 'ai-voice' && (
            <>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link href="#" onClick={() => onViewChange('ai-voice')} className="text-gray-600 hover:text-gray-900">
                    AI Voice
                  </Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
